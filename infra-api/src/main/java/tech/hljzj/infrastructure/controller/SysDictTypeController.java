...
import tech.hljzj.infrastructure.vo.SysDictType.SysDictTypeNewVo;
import tech.hljzj.infrastructure.vo.SysDictType.SysDictTypeQueryVo;
import tech.hljzj.infrastructure.vo.SysDictType.SysDictTypeUpdateVo;
...
public class SysDictTypeController extends BaseController {
    ...
    public R<SysDictType> entityGet(@PathVariable Serializable id) {
    ...
    @PostMapping("/insert")
    public R<SysDictType> entityCreate(@RequestBody @Validated SysDictTypeNewVo entity) {
    ...
    @PostMapping("/update")
    public R<SysDictType> entityUpdate(@RequestBody @Validated SysDictTypeUpdateVo entity) {
        SysDictType dto = entity.toDto();
    ...
    public void export(@RequestBody SysDictTypeQueryVo query, HttpServletResponse response) {
        List<SysDictType> listDto = this.service.list(query);

        List<SysDictTypeListVo> listVo = new ArrayList<>();
        for (SysDictType item : listDto) {
            SysDictTypeListVo vo = new SysDictTypeListVo();
            listVo.add(vo.fromDto(item));
        }

        ExcelUtil.exportExcel(response, listVo, "数据", SysDictTypeListVo.class);
    }
    ...
    public R<List<ExcelUtil.FailRowWrap<SysDictTypeListVo>>> importData(@RequestPart MultipartFile file) throws IOException {
        return R.ok(ExcelUtil.readExcel(file.getInputStream(), SysDictTypeListVo.class, SysDictTypeListVo -> {
            this.service.entityCreate(SysDictTypeListVo.toDto());
        }));
    }

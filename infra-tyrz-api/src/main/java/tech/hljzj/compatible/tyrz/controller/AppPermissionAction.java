...
import tech.hljzj.compatible.tyrz.vo.permission.PermissionTree;
...
//        Permission byPermission = sysMenuService.list(permission.toQuery());
//        if (byPermission == null) {
//            return ApiMsgUtil.failure(BaseCode.FAILURE, ServerErrorCode.INDEXNOTFOUNT);
//        }
//        Set<String> roleIds = service(RolePermissionService.class).findRoleIdsByPermissionInfo(permission.getPermissionId());
//        Set<String> userIds = service(UserRoleService.class).findUserIdByRoleIdIn(roleIds);
//        return ApiMsgUtil.success(service(UserInfoService.class).findByUserIdIn(userIds));
        return null;
    }
...
//
    ...
    private Object findByPermissionParent(String permissionInfo) {
        ...
        query.setParentId(permissionInfo);
        query.setOwnerAppId(AppHelper.getLoginApp(request));
        SysMenu menu = sysMenuService.getOne(query.buildQueryWrapper());
        if (menu != null) {
